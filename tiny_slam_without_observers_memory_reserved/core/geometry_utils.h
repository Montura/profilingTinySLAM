...
public: // methods
  ...
  // }
  DiscreteLine2D(const Point &start, const Point &end, std::vector<Point> & pts) {
    generatePointsWithBresenham(start.x, start.y, end.x, end.y, pts);
  }
...
private:
  ...
  void generatePointsWithBresenham(int x1, int y1, int x2, int y2, std::vector<Point> & pts) {
